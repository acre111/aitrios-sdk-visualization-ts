...
export const preDeserialize = (inferenceData: any) => {
  const o = inferenceData[0].inferences[0].O
  const data = deserialize(o)
  if (data) {
    inferenceData[0].inferences[0].O = data
    return inferenceData
  } else {
    console.log('deserializedInferenceData is null')
  }
}
...
export default function deserialize (inferenceData) {
    ...
    if (inferenceData) {
      decodedData = Buffer.from(JSON.stringify(inferenceData), 'base64')
    } else {
