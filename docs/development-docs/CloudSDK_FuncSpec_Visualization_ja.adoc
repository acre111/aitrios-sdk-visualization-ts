= pass:[<br/>]Cloud SDK Visualization 機能仕様書
:toc:
:toclevels: 1
...
|AITRIOS^TM^ \| Consoleへのアクセス方法を提供するSDK
...
|画像
|Vision and Sensing Applicationからの出力のうち、カメラがとらえているイメージデータ
...
* ベースAIモデルはObjectDetectionのみ対応する。
...
[#_Input-DeviceID]
.Device ID入力画面
image::./ScreenSpec_visualizationTool_deviceid.png[Device ID入力画面仕様]
. 推論結果を確認したい *"Device ID"* を入力して設定する。
.  *"Device ID"* を確定し結果確認画面に遷移する。
..  *"Device ID"* に *"mock"* と入力した場合、Consoleへは接続せず、Visualizationがダミー動作をする。

.RealtimeMode画面
image::./ScreenSpec_visualizationTool_realtime.png[RealtimeMode画面仕様]
...
.HistoryMode画面
image::./ScreenSpec_visualizationTool_history.png[HistoryMode画面仕様]
...
* ConsoleのWebUIから、Command Parameter Fileを下記の設定にしておく。
